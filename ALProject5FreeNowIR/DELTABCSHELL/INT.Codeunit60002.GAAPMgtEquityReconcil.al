codeunit 60002 "GAAP Mgt. - Equity Reconcil."
{
    ...
    begin
        ...
        /*
            if gblnBottomUp then begin
                grecGenJnlTemplate[3].SetRange(Type, grecGenJnlTemplate[3].Type::"Group Adjustments");
    ...
    */
    ...
    begin
        If ptmpEquityReconLine.FindFirst() then begin
            ...
            // Tax
            gblnTax := true;
            //lfcnInsertLineSubtotal(ptmpEquityReconLine, Type::Total, StrSubstNo(txt60003, Date2DMY(gdatStart, 3)), 0D, 0D);
            ...
            //lfcnInsertLineSubtotal(ptmpEquityReconLine, Type::Total, StrSubstNo(txt60006, Date2DMY(gdatStart, 3)), 0D, 0D);
        end;
    end;
    ...
    begin
        with ptmpEquityReconLine do begin
            case Type of
                Type::"Corp. Account Summary":
                    begin
                        if pintFieldNo in [FieldNo("Document No. (Start Balance)"), FieldNo("Document No. (Net Change)")] then
                            exit;
                        ...
                        if Code = '' then begin
                            ...
                            if pintFieldNo = FieldNo("Net Change (P&L)") then // Retained Earnings MP 22-06-12 Replaces above
                                                                              // MP 03-12-13 >>
                                if not gblnCorpAccInUse then
                                ...
                                if (pintFieldNo = FieldNo("End Balance")) and (not gblnPeriodClosed) then // MP 03-12-13 Replaces above
                                    exit;
                ...
                Type::Entries:
                    begin
                        ...
                        if Subtotal then
                            ...
                            lrecGLEntry.SetRange("Equity Correction Code", Code);

                        // MP 03-12-13 >>
                        if not gblnCorpAccInUse then
                            // Set "G/L Account No." filter
                            case pintFieldNo of
                                FieldNo("Start Balance"), FieldNo("End Balance"),
                    FieldNo("Document No. (Start Balance)"), FieldNo("Document No. (Net Change)"):
                    ...
                    FieldNo("Document No. (Start Balance)"), FieldNo("Document No. (Net Change)"):
                        ...
                        case pintFieldNo of
                            FieldNo("Start Balance"), FieldNo("Document No. (Start Balance)"):
                                lrecGLEntry.SetFilter("Document No.", "Document No. (Start Balance)");

                            FieldNo("Net Change (P&L)"), FieldNo("Document No. (Net Change)"):
                                lrecGLEntry.SetFilter("Document No.", "Document No. (Net Change)");
                        ...
                        // Set "Posting Date" filter
                        if Year <> 0 then
                            lrecGLEntry.SetRange("Posting Date", "Year Start Date", "Year End Date")
                        else
                            case pintFieldNo of
                                FieldNo("Start Balance"), FieldNo("Document No. (Start Balance)"):
                                    // MP 08-05-14 Same filter regardless of TB-to-TB or Transactional >>
                                    ...
                                    lrecGLEntry.SetFilter("Posting Date", '<%1', gdatStart); // MP 10-01-13 Reinstated line
                                                                                             ...
                                                                                             //  lrecGLEntry.SETFILTER("Posting Date",'<=%1',CALCDATE('-1D',gdatStart)); // SB 12-06-12 Replaces above
                                                                                             // MP 08-05-14 <<
                                FieldNo("Net Change (P&L)"), FieldNo("Net Change (Equity)"), FieldNo("Document No. (Net Change)"):
                                    lrecGLEntry.SetRange("Posting Date", gdatStart, gdatEnd);
    ...
    begin
        with ptmpEquityReconLine do begin
            ...
            Type := pintType;
            Code := pcodCode;
            Description := ptxtDescription;
            "Entry Description" := ptxtEntryDescription;
            "Start Balance" := pdecStartBalance;
            "Net Change (P&L)" := pdecNetChangePL;
            "Net Change (Equity)" := pdecNetChangeEquity;
            "End Balance" := pdecStartBalance + pdecNetChangePL + pdecNetChangeEquity;
            "Document No. (Start Balance)" := pcodDocNoStartBalance;
            "Document No. (Net Change)" := pcodDocNoNetChange;
            Year := pintYear;
            "Year Start Date" := pdatStartDate;
            "Year End Date" := pdatEndDate;
            Subtotal := pblnSubtotal;
            Statutory := gblnStatutory;
            Tax := gblnTax;

            Insert;

            if not Subtotal then begin
        ...
        with ltmpEntryBuffer do begin
                            ...
                            if NormalDate(lrecGLEntry."Posting Date") = lrecGLEntry."Posting Date" then begin // Exclude Closing Dates
                                ...
                                if goptGroupBy = goptGroupBy::"Equity Correction Code" then
                                    ...
                                    if lrecGLEntry."Posting Date" >= gdatStart then begin
                                        Year := lintCurrYear;
                                        "Year Start Date" := gdatStart;
                                        "Year End Date" := gdatEnd;
                                    end else begin
                                        Year := Date2DMY(lrecGLEntry."Posting Date", 3);
                                        gmdlGAAPMgt.gfcnGetAccPeriodFilter(lrecGLEntry."Posting Date", "Year Start Date", "Year End Date");
                                    end;

                                "Equity Correction Code" := lrecGLEntry."Equity Correction Code";

                                if goptGroupBy = goptGroupBy::Year then
                                    "Document No." := lrecGLEntry."Document No.";
                                ...
                                if not Find then begin
                                    ...
                                    if (goptGroupBy = goptGroupBy::"Equity Correction Code") and (lrecGLEntry."Posting Date" < gdatStart) then
                                        "Start Balance" := ldecAmount // MP 03-12-13 Replaces lrecGLEntry.Amount
                                    else

                                        // MP 06-12-12 <<

                                        if lintI = 1 then
                                            "Net Change (P&L)" := ldecAmount // MP 03-12-13 Replaces lrecGLEntry.Amount
                                        else
                                            "Net Change (Equity)" := ldecAmount; // MP 03-12-13 Replaces lrecGLEntry.Amount;

                                    if goptGroupBy = goptGroupBy::Year then
                                        "Entry Description" := lrecGLEntry.Description;
                                    ...
                                    if (goptGroupBy = goptGroupBy::"Equity Correction Code") and (lrecGLEntry."Posting Date" < gdatStart) then
                                        "Start Balance" += ldecAmount // MP 21-01-13 Changed from := to += // MP 03-12-13 Replaces lrecGLEntry.Amount
                                    else
            ...
            if FindSet then begin
                ...
                repeat
                    ...
                    if ltmpPrevEntryBuffer.Year <> Year then
                    ...
                    if "Equity Correction Code" <> '' then
                        lrecEquityCorrectionCode.Get("Equity Correction Code")
                    else
                        Clear(lrecEquityCorrectionCode);

                    // Get Entry Description
                    if goptGroupBy = goptGroupBy::Year then
                        if lrecEquityReconEntryDesc.Get("Equity Correction Code", Year, "Document No.") then
                            "Entry Description" := lrecEquityReconEntryDesc.Description;

                    // Insert Entry
                    if (Year = lintCurrYear) or (goptGroupBy = goptGroupBy::"Equity Correction Code") then
                        lfcnInsertLine(ptmpEquityReconLine, ptmpEquityReconLine.Type::Entries,
                          "Equity Correction Code", lrecEquityCorrectionCode.Description, "Entry Description",
                          //          0,-"Net Change (P&L)",-"Net Change (Equity)",
                          -"Start Balance", -"Net Change (P&L)", -"Net Change (Equity)", // MP 06-12-12 Replaces above line
                          '', "Document No.", Year, "Year Start Date", "Year End Date", false)
                    else
                        lfcnInsertLine(ptmpEquityReconLine, ptmpEquityReconLine.Type::Entries,
                          "Equity Correction Code", lrecEquityCorrectionCode.Description, "Entry Description",
                          -"Net Change (P&L)" - "Net Change (Equity)", 0, 0,
                          "Document No.", '', Year, "Year Start Date", "Year End Date", false);
            ...
            // Insert Subtotal
            // MP 18-02-16 Do not create subtotals by year, only blank line >>
            if (Year <> lintCurrYear) and (Year <> 0) then
                //  lfcnInsertLineSubtotal(ptmpEquityReconLine,ptmpEquityReconLine.Type::Entries,
                //    STRSUBSTNO(ltxtSubTotalCaption,ltmpPrevEntryBuffer.Year),
                //    ltmpPrevEntryBuffer."Year Start Date",ltmpPrevEntryBuffer."Year End Date");
                lfcnInsertLineBlank(ptmpEquityReconLine);
    ...
    begin
        ...
        with lrecCorpGLAccount do begin
            SetRange("Account Class", "Account Class"::Equity, "Account Class"::"P&L");
            FindFirst;

            SetRange("Account Class");
            SetFilter("No.", '%1..', "No.");

            FindSet;
            lrecPrevCorpGLAccount."Account Class" := "Account Class";
            repeat
                if lrecPrevCorpGLAccount."Account Class" <> "Account Class" then begin
                    if lrecPrevCorpGLAccount."No." <> '' then begin
                        ...
                        case "Account Class" of
                            "Account Class"::Equity:
                                gtxtGLAccountFilterEquity += "No.";
                            "Account Class"::"P&L":
                                gtxtGLAccountFilterPL += "No.";
                        end;
    ...
    begin
        ...
        with lrecGLAccount do begin
            ...
            FindSet;
            lrecPrevGLAccount."Account Class" := "Account Class";
            repeat
                if lrecPrevGLAccount."Account Class" <> "Account Class" then begin
                    ...
                    if NormalDate(ltmpAdjmtEntryBuffer."Posting Date") = ltmpAdjmtEntryBuffer."Posting Date" then begin // Exclude Closing Dates
                        ...
                        if goptGroupBy = goptGroupBy::"Equity Correction Code" then
                            ...
                            if ltmpAdjmtEntryBuffer."Posting Date" >= gdatStart then begin
                                ...
                                Year := Date2DMY(ltmpAdjmtEntryBuffer."Posting Date", 3);
                                gmdlGAAPMgt.gfcnGetAccPeriodFilter(ltmpAdjmtEntryBuffer."Posting Date", "Year Start Date", "Year End Date");
                            end;

                        "Equity Correction Code" := ltmpAdjmtEntryBuffer."Equity Correction Code";

                        if goptGroupBy = goptGroupBy::Year then
                            "Document No." := ltmpAdjmtEntryBuffer."Document No.";
                        ...
                        if not Find then begin
                            if (goptGroupBy = goptGroupBy::"Equity Correction Code") and (ltmpAdjmtEntryBuffer."Posting Date" < gdatStart) then
                                "Start Balance" := ldecAmount
                            else
                                if pintI = 1 then
                                    "Net Change (P&L)" := ldecAmount
                                else
                                    "Net Change (Equity)" := ldecAmount;

                            if goptGroupBy = goptGroupBy::Year then
                                "Entry Description" := ltmpAdjmtEntryBuffer.Description;
