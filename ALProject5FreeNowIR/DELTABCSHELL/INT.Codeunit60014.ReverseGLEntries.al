codeunit 60014 "Reverse G/L Entries"
{
    ...
    begin
        ...
        with lrecGLEntry do begin
            if lrecGLRegister.FindSet then
                repeat
                    SetRange("Entry No.", lrecGLRegister."From Entry No.", lrecGLRegister."To Entry No.");
                    ...
                    then
                        ...
                        if not IsEmpty then begin // Entries exist with "Global Dimension 1 Code"
                            SetRange("Global Dimension 1 Code", '');
                            if not IsEmpty then // Mixture of blank and non-blank entries
                                SetRange("Global Dimension 1 Code");
                    ...
                    if FindSet then begin
                        repeat
                            if ldatPrevPostingDate <> "Posting Date" then begin
                                // MP 25-11-13 >>
                                ...
                                lfrfField.SetRange("Posting Date", 99991231D);
                            ...
                            lrecGenJnlLine."Account No." := "G/L Account No.";
                            lrecGenJnlLine."Posting Date" := ldatPostingDateReversal;
                            lrecGenJnlLine."Document No." := "Document No.";
                            lrecGenJnlLine.Description := Description;
                            lrecGenJnlLine.Validate(Amount, -Amount);
                            // MP 17-11-14 >>
                            if lrecGenJnlLine.Amount = 0 then
                                lrecGenJnlLine."Allow Zero-Amount Posting" := true;
                            // MP 17-11-14 <<
                            lrecGenJnlLine."Shortcut Dimension 1 Code" := "Global Dimension 1 Code";
                            lrecGenJnlLine."Shortcut Dimension 2 Code" := "Global Dimension 2 Code";
                            lrecGenJnlLine."Business Unit Code" := "Business Unit Code";
                            lrecGenJnlLine."External Document No." := "External Document No.";
                            lrecGenJnlLine."Corporate G/L Account No." := "Corporate G/L Account No.";
                            ...
                            lrecGenJnlLine."Dimension Set ID" := "Dimension Set ID";
                            lmdlGenJnlPostLine.RunWithCheck(lrecGenJnlLine);
                            // MP 17-11-14 Upgraded to NAV 2013 R2 <<

                            ldatPrevPostingDate := "Posting Date";
        ...
        if lrecGLEntry.FindSet then begin
            ...
            repeat
                ...
                with lrecGLEntry do begin
                    ...
                    if "Global Dimension 1 Code" = pcodGenJnlTemplate[1]."Shortcut Dimension 1 Code" then
                        lintJnlNo := 1
                    else
                        lintJnlNo := 2;
                    ...
                    if pcodJnlBatchName[lintJnlNo] <> '' then begin
                        ...
                        lrecGenJournalLine[lintJnlNo].Validate("Account No.", "G/L Account No.");
                        lrecGenJournalLine[lintJnlNo].Validate("Corporate G/L Account No.", "Corporate G/L Account No.");
                        lrecGenJournalLine[lintJnlNo].Validate("Posting Date", p_datReversalPostingDate);
                        lrecGenJournalLine[lintJnlNo].Validate("Document No.", "Document No.");
                        lrecGenJournalLine[lintJnlNo].Validate(Description, Description);
                        lrecGenJournalLine[lintJnlNo].Validate(Amount, -Amount);
                        if lrecGenJournalLine[lintJnlNo].Amount = 0 then
                            lrecGenJournalLine[lintJnlNo].Validate("Allow Zero-Amount Posting", true);

                        lrecGenJournalLine[lintJnlNo].Validate("Shortcut Dimension 1 Code", "Global Dimension 1 Code");
                        lrecGenJournalLine[lintJnlNo].Validate("Shortcut Dimension 2 Code", "Global Dimension 2 Code");
                        lrecGenJournalLine[lintJnlNo].Validate("Business Unit Code", "Business Unit Code");
                        lrecGenJournalLine[lintJnlNo].Validate("External Document No.", "External Document No.");
                        lrecGenJournalLine[lintJnlNo].Validate("Source Code", lrecSourceCodeSetup."GAAP/Tax Reversal");
                        lrecGenJournalLine[lintJnlNo].Validate("Description (English)", "Description (English)");

                        lrecGenJournalLine[lintJnlNo].Validate("GAAP Adjustment Reason", "GAAP Adjustment Reason");
                        if "Adjustment Role" = "Adjustment Role"::Auditor then
                            lrecGenJournalLine[lintJnlNo].Validate("Adjustment Role", "Adjustment Role"::EY)
                        else
                            lrecGenJournalLine[lintJnlNo].Validate("Adjustment Role", "Adjustment Role");

                        lrecGenJournalLine[lintJnlNo].Validate("Tax Adjustment Reason", "Tax Adjustment Reason");
                        lrecGenJournalLine[lintJnlNo].Validate("Equity Correction Code", "Equity Correction Code");

                        lrecGenJournalLine[lintJnlNo].Validate("Dimension Set ID", "Dimension Set ID");

                        lrecGenJournalLine[lintJnlNo].Validate("Entry No. to Reverse", "Entry No.");
