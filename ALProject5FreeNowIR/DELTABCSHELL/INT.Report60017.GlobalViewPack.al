report 60017 "Global View Pack"
{
    ...
    begin
        with gtmpExcelBuffer do begin
            Init;
            Validate("Row No.", pintRowNo);
            Validate("Column No.", pintColumnNo);
            "Cell Value as Text" := ptxtCellValue;
            Formula := '';
            Bold := pblnBold;
            Italic := pblnItalic;
            Underline := pblnUnderLine;
            NumberFormat := ptxtFormat;
            "Cell Type" := poptCellType;
            Insert;
        end;
    end;
