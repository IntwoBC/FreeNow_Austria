codeunit 60001 "GAAP Mgt. - Global View"
{
    ...
    begin
        ...
        with ptmpCorpGLAcc do begin
            CopyFilter("Date Filter", lrecCorpGLAcc."Date Filter"); // Retain Date Filter
            Init;
            Reset;
            DeleteAll;
            lrecCorpGLAcc.CopyFilter("Date Filter", "Date Filter");
            //SETCURRENTKEY("Search Name");
            //SETCURRENTKEY("Financial Statement Code"); // MP 23-04-14 Replaces above line
            SetCurrentKey(Indentation); // MP 25-11-15 Replaces above line
            ...
            if not gmdlCompanyTypeMgt.gfcnCorpAccInUse or gblnShowLocalGLAcc then begin // MP 23-04-14 Added gblnShowLocalGLAcc
                ...
                if lrecGLAcc.FindSet then
                    repeat
                        TransferFields(lrecGLAcc);

                        "Local G/L Account No." := lrecGLAcc."No.";
                        // MP 23-04-14 >>
                        //"Search Name" := lcodSequenceNo;
                        "No." := lcodSequenceNo;
                        "Search Name" := lrecGLAcc."Corporate G/L Account No.";
                        //"Financial Statement Code" := lcodSequenceNo;
                        Evaluate(Indentation, lcodSequenceNo); // MP 25-11-15 Replaces above line
                                                               // MP 23-04-14 <<
                ...
                if lrecCorpGLAcc.FindSet then
                    repeat
                        ...
                        Evaluate(Indentation, lcodSequenceNo); // MP 03-11-15 Replaces above line
                        lcodSequenceNo := IncStr(lcodSequenceNo);
                        Insert;
    ...
    begin
        ...
        with precGLEntry do begin
            SetCurrentKey("Corporate G/L Account No.", "Global Dimension 1 Code", "Global Dimension 2 Code", "Adjustment Role", "GAAP Adjustment Reason", "Posting Date");

            SetRange("Corporate G/L Account No.", precCorpGLAcc."No.");
            SetRange("Global Dimension 1 Code", pcodShortcutDim1Code);
            SetFilter("GAAP Adjustment Reason", ptxtGAAPAdjmtReasonFilter);
            SetFilter("Posting Date", ptxtDateFilter);
            precCorpGLAcc.CopyFilter("Global Dimension 2 Filter", "Global Dimension 2 Code");
            precCorpGLAcc.CopyFilter("Business Unit Filter", "Business Unit Code");
        end;
    end;
    ...
    begin
        ...
        with ptmpCorpGLAcc do begin
            ...
            CopyFilter("Date Filter", precGLEntry."Posting Date");
