codeunit 70001 "MyTaxi CRM Interface WS"
{
    ...
    // #MyTaxi.IT.CRE.INT01.005 15/02/2019 CCFR.SDE :
    //   Retreive "Unique Office Code" & "PEC E-Mail" from CRM App
    ...
    // PK 12-08-24 EY-MYIT0002 Case CS0806754 / Feature 6079423
    // Function added
    //   - GetMasterDataByID
    // Functions modified:
    //   - GetMasterData
    ...
    begin
        ...
        if CalledFromMasterDataByID then begin
            if GuiAllowed then begin
                ...
                if CustomerIDDialog.RunModal() = Action::OK then
                    // Retrieve the Customer IDs, From Date, and To Date from the dialog page
                    CustomerIDsToGet := CustomerIDDialog.GetCustomerIDsToGet();
            end;

            if CustomerIDsToGet = '' then
                exit;
        end else begin
            // EY-MYIT0002 <<
            if GuiAllowed then begin
        ...
        foreach JToken in CustomerList do begin
            ...
            TmpMyTaxiCRMInterfaceRecords."Entry No." := TempEntryNo;
            ...
            if CustomerContent.Get('number', JToken) then
                TmpMyTaxiCRMInterfaceRecords.number := JToken.AsValue().AsInteger();

            if CustomerContent.Get('name', JToken) then
            ...
            if CustomerContent.Get('city', JToken) then
            ...
            if CustomerContent.Get('email', JToken) then
                TmpMyTaxiCRMInterfaceRecords.email := CopyStr(JToken.AsValue().AsText(), 1, MaxStrLen(TmpMyTaxiCRMInterfaceRecords.email));

            if CustomerContent.Get('contact', JToken) then
            ...
            if CustomerContent.Get('address1', JToken) then
                TmpMyTaxiCRMInterfaceRecords.address3 := CopyStr(JToken.AsValue().AsText(), 1, MaxStrLen(TmpMyTaxiCRMInterfaceRecords.address3));

            if CustomerContent.Get('contact', JToken) then
                TmpMyTaxiCRMInterfaceRecords.contact2 := CopyStr(JToken.AsValue().AsText(), 1, MaxStrLen(TmpMyTaxiCRMInterfaceRecords.contact2));

            if CustomerContent.Get('contact', JToken) then
                TmpMyTaxiCRMInterfaceRecords.contact3 := CopyStr(JToken.AsValue().AsText(), 1, MaxStrLen(TmpMyTaxiCRMInterfaceRecords.contact3));
            // MyTaxi.W1.CRE.INT01.001 >>
            ...
            //if CustomerContent.Get('italyCodiceUnivoco', JToken) then
            //TmpMyTaxiCRMInterfaceRecords.italyCodiceUnivoco := JToken.AsValue().AsText();
        ...
        if TmpMyTaxiCRMInterfaceRecords.FindSet then
            repeat
                ...
                if not Customer.Get(Format(MyTaxiCRMInterfaceRecords.id)) then
                    ...
                    if TmpMyTaxiCRMInterfaceRecords.name <> Customer.Name then
                        bInsert := true;
                    if TmpMyTaxiCRMInterfaceRecords.address1 <> Customer.Address then
                        bInsert := true;
                    if TmpMyTaxiCRMInterfaceRecords.city <> Customer.City then
                        bInsert := true;
                    if TmpMyTaxiCRMInterfaceRecords.zip <> Customer."Post Code" then
                        bInsert := true;
                    if TmpMyTaxiCRMInterfaceRecords.country <> Customer."Country/Region Code" then
                        bInsert := true;
                    if TmpMyTaxiCRMInterfaceRecords.tele1 <> Customer."Phone No." then
                        bInsert := true;
                    if TmpMyTaxiCRMInterfaceRecords.email <> Customer."E-Mail" then
                        bInsert := true;
                    if TmpMyTaxiCRMInterfaceRecords.contact <> Customer.Contact then
                        bInsert := true;
                    if TmpMyTaxiCRMInterfaceRecords.vatNo <> Customer."VAT Registration No." then
                        bInsert := true;
                    // MyTaxi.W1.CRE.INT01.009 <<
                    if TmpMyTaxiCRMInterfaceRecords.bankAccountNumber <> '' then begin
                        if not CustomerBankAccount.Get(Format(MyTaxiCRMInterfaceRecords.id), TmpMyTaxiCRMInterfaceRecords."NAV Bank Account Code") then
                            ...
                            if TmpMyTaxiCRMInterfaceRecords.accountHolder <> CustomerBankAccount.Name then
                                bInsert := true;
                            if TmpMyTaxiCRMInterfaceRecords.iban <> CustomerBankAccount.IBAN then
                                bInsert := true;
                            if TmpMyTaxiCRMInterfaceRecords.bic <> CustomerBankAccount."SWIFT Code" then
                                bInsert := true;
                            if TmpMyTaxiCRMInterfaceRecords.bankAccountNumber <> CustomerBankAccount."Bank Account No." then
                                bInsert := true;
                            if TmpMyTaxiCRMInterfaceRecords.sortCode <> CustomerBankAccount."Bank Branch No." then
                                bInsert := true;
                        end;
                    end;
                    // MyTaxi.W1.CRE.INT01.009 >>
                ...
                if bInsert then begin
                    ...
                    MyTaxiCRMInterfaceRecords.Insert(true);
                end;
    ...
    begin
        ...
        //TransferTmpInvToInvoiceRec(TmpMyTaxiCRMInterfaceRecords, TmpMyTaxiCRMInterfSubRecords);//Changed
    end;
    ...
    begin
        //Added
        if Invoice.Get('id', JToken) then begin
        ...
        if Invoice.Get('sumNetValue', JToken) then begin
            ...
            if Invoice.Get('sumTaxValue', JToken) then
                Evaluate(TmpMyTaxiCRMInterfaceRecords.sumTaxValue, JToken.AsValue().AsText());
            if Invoice.Get('sumGrossValue', JToken) then
                Evaluate(TmpMyTaxiCRMInterfaceRecords.sumGrossValue, JToken.AsValue().AsText());
        end;
        if Invoice.Get('discountCommissionNet', JToken) then begin
            ...
            if Invoice.Get('discountCommissionTax', JToken) then
                Evaluate(TmpMyTaxiCRMInterfaceRecords.discountCommissionTax, JToken.AsValue().AsText());
            if Invoice.Get('discountCommissionGross', JToken) then
                Evaluate(TmpMyTaxiCRMInterfaceRecords.discountCommissionGross, JToken.AsValue().AsText());
        end;
        if Invoice.Get('netCommission', JToken) then begin
            Evaluate(TmpMyTaxiCRMInterfaceRecords.netCommission, JToken.AsValue().AsText());
            if Invoice.Get('taxCommission', JToken) then
                Evaluate(TmpMyTaxiCRMInterfaceRecords.taxCommission, JToken.AsValue().AsText());
            if Invoice.Get('grossCommission', JToken) then
                Evaluate(TmpMyTaxiCRMInterfaceRecords.grossCommission, JToken.AsValue().AsText());
        end;
        if Invoice.Get('netHotelValue', JToken) then begin
            Evaluate(TmpMyTaxiCRMInterfaceRecords.netHotelValue, JToken.AsValue().AsText());
            if Invoice.Get('taxHotelValue', JToken) then
                Evaluate(TmpMyTaxiCRMInterfaceRecords.taxHotelValue, JToken.AsValue().AsText());
            if Invoice.Get('grossHotelValue', JToken) then
                Evaluate(TmpMyTaxiCRMInterfaceRecords.grossHotelValue, JToken.AsValue().AsText());
        end;
        if Invoice.Get('netInvoicingFee', JToken) then begin
            ...
            if Invoice.Get('taxInvoicingFee', JToken) then
                Evaluate(TmpMyTaxiCRMInterfaceRecords.taxInvoicingFee, JToken.AsValue().AsText());
            if Invoice.Get('grossInvoicingFee', JToken) then
                Evaluate(TmpMyTaxiCRMInterfaceRecords.grossInvoicingFee, JToken.AsValue().AsText());
        end;
        if Invoice.Get('netPayment', JToken) then
            Evaluate(TmpMyTaxiCRMInterfaceRecords.netPayment, JToken.AsValue().AsText());
        if Invoice.Get('netPaymentFeeMP', JToken) then begin
            ...
            if Invoice.Get('taxPaymentFeeMP', JToken) then
                Evaluate(TmpMyTaxiCRMInterfaceRecords.taxPaymentFeeMP, JToken.AsValue().AsText());
            if Invoice.Get('grossPaymentFeeMP', JToken) then
                Evaluate(TmpMyTaxiCRMInterfaceRecords.grossPaymentFeeMP, JToken.AsValue().AsText());
        end;
        if Invoice.Get('netPaymentFeeBA', JToken) then begin
            ...
            if Invoice.Get('taxPaymentFeeBA', JToken) then
                Evaluate(TmpMyTaxiCRMInterfaceRecords.taxPaymentFeeBA, JToken.AsValue().AsText());
        ...
        if TmpMyTaxiCRMInterfaceRecords.netPayment <> 0 then
            TmpMyTaxiCRMInterfaceRecords."NAV Payment Status" := TmpMyTaxiCRMInterfaceRecords."NAV Payment Status"::Imported;
        // MyTaxi.W1.CRE.INT01.015 >>
        TmpMyTaxiCRMInterfaceRecords.Insert;
...
=======
                        if Invoice.Get('taxCredit', JToken) then
                            Evaluate(TmpMyTaxiCRMInterfSubRecords.taxCredit, JToken.AsValue().AsText());
                        if Invoice.Get('grossCredit', JToken) then
>>>>>>> 41feae924f0965f985b7147a4021ea71e5688780
                    ...
                    if (TmpMyTaxiCRMInterfaceRecords."NAV Credit Memo Status" <> TmpMyTaxiCRMInterfaceRecords."NAV Credit Memo Status"::Imported) then
                        if (TmpMyTaxiCRMInterfSubRecords.netCredit > 0) and
                          MyTaxiCRMIntPostingMap.Get(TmpMyTaxiCRMInterfaceRecords.invoiceType, TmpMyTaxiCRMInterfSubRecords.accountNumber) and
                          (MyTaxiCRMIntPostingMap."Document Type" = MyTaxiCRMIntPostingMap."Document Type"::"Credit Memo") then begin
    ...
    begin
        // Extract parameters
        if not Parameters.Get('baseurl', BaseUrl) then
            Error('Base URL is required.');
        if not Parameters.Get('path', Path) then
            Error('Path is required.');
        if not Parameters.Get('restmethod', RestMethod) then
            Error('REST method is required.');
        ...
        // Set common headers
        if Parameters.Get('accept', JsonBody) then
            HttpClient.DefaultRequestHeaders.Add('Accept', JsonBody);
        ...
        if not HttpResponseMessage.IsSuccessStatusCode() then
            Error('HTTP request failed with status code %1.', StatusCode);
    ...
    begin
        ...
        if not JsonObject.Get('customers', JsonToken) then
            Error('Invalid JSON format. "customers" array is missing.');
        ...
        foreach JsonToken in JsonArray do begin
            ...
            if CustomerContent.Get('company', JsonToken) then
                TmpMyTaxiCRMInterfaceRecords.company := JsonToken.AsValue().AsText();
            if CustomerContent.Get('id', JsonToken) then
                Evaluate(TmpMyTaxiCRMInterfaceRecords.id, JsonToken.AsValue().AsText());
            if CustomerContent.Get('number', JsonToken) then
                Evaluate(TmpMyTaxiCRMInterfaceRecords.number, JsonToken.AsValue().AsText());
            if CustomerContent.Get('name', JsonToken) then
                TmpMyTaxiCRMInterfaceRecords.name := CopyStr(JsonToken.AsValue().AsText(), 1, MaxStrLen(TmpMyTaxiCRMInterfaceRecords.name));
            if CustomerContent.Get('orgNo', JsonToken) then
                TmpMyTaxiCRMInterfaceRecords.orgNo := JsonToken.AsValue().AsText();
            if CustomerContent.Get('address1', JsonToken) then
                TmpMyTaxiCRMInterfaceRecords.address1 := JsonToken.AsValue().AsText();
            if CustomerContent.Get('city', JsonToken) then
                TmpMyTaxiCRMInterfaceRecords.city := JsonToken.AsValue().AsText();
            if CustomerContent.Get('zip', JsonToken) then
                TmpMyTaxiCRMInterfaceRecords.zip := JsonToken.AsValue().AsText();
            if CustomerContent.Get('country', JsonToken) then
                TmpMyTaxiCRMInterfaceRecords.country := JsonToken.AsValue().AsText();
            if CustomerContent.Get('tele1', JsonToken) then
                TmpMyTaxiCRMInterfaceRecords.tele1 := JsonToken.AsValue().AsText();
            if CustomerContent.Get('email', JsonToken) then
                TmpMyTaxiCRMInterfaceRecords.email := JsonToken.AsValue().AsText();
            if CustomerContent.Get('contact', JsonToken) then
                TmpMyTaxiCRMInterfaceRecords.contact := CopyStr(JsonToken.AsValue().AsText(), 1, MaxStrLen(TmpMyTaxiCRMInterfaceRecords.contact));
            if CustomerContent.Get('vatNo', JsonToken) then
                TmpMyTaxiCRMInterfaceRecords.vatNo := JsonToken.AsValue().AsText();
    ...
    begin
        ...
        if not JsonObject.Get('invoices', JsonToken) then
            Error('Invalid JSON format. "invoices" array is missing.');
