codeunit 63001 "P:G/L Account Card"
{
    ...
    begin
        with Rec do begin
            if ("Financial Statement Code" = '') and ("No." <> '') then
                Error(txt60000, FieldCaption("Financial Statement Code"), TableCaption, FieldCaption("No."), "No.")
            else begin
                CalcFields(BottomUp, CorpAccInUse);
                if BottomUp and CorpAccInUse and ("Corporate G/L Account No." = '') and ("No." <> '') then
                    Error(txt60000, FieldCaption("Corporate G/L Account No."), TableCaption, FieldCaption("No."), "No.")
                else
                    AllowModify := true;
            end;
        end;
