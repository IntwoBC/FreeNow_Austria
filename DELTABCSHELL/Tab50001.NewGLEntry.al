table 50001 "I2I G/L Entry"
{
    ...
    fields
    {
        ...
        {
            CalcFormula = Exist("G/L Entry Document Link" WHERE("Transaction No." = FIELD("Transaction No."), "Document No." = FIELD("Document No.")));
            Caption = 'Links';
            Description = 'MP 18-01-12';
            Editable = false;
            FieldClass = FlowField;
    ...
    begin
        if ((Amount > 0) and (not Correction)) or ((Amount < 0) and Correction) then begin
            "Debit Amount" := Amount;
            "Credit Amount" := 0
        end
        else begin
        ...
        if (("Additional-Currency Amount" > 0) and (not Correction)) or (("Additional-Currency Amount" < 0) and Correction) then begin
    ...
    begin
        ...
        if (GenJnlLine."Account Type" = GenJnlLine."Account Type"::"IC Partner") or (GenJnlLine."Bal. Account Type" = GenJnlLine."Bal. Account Type"::"IC Partner") then "Source Type" := "Source Type"::" ";
    ...
    begin
        ...
        if not GLAccount.Get("G/L Account No.") then exit;
    ...
    var
        GLAccount: Record "G/L Account";
    begin
        if IsNullGuid("Account Id") then exit;
        if not GLAccount.GetBySystemId("Account Id") then exit;
