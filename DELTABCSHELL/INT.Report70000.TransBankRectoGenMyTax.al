report 70000 "Trans. Bank Rec. to Gen. MyTax"
{
    ...
    // #MyTaxi.W1.CRE.AR01.002 04/07/2017 CCFR.SDE : MyTaxi CRM Interface
    ...
    // #MyTaxi.W1.CRE.AR01.004 07/12/2017 CCFR.SDE : MyTaxi CRM Interface
    //   Apply only remaining amount on the customer ledger entry
                    ...
                    if "Bank Acc. Reconciliation Line"."Document No." <> '' then
                        if SalesInvoiceHeader.Get("Bank Acc. Reconciliation Line"."Document No.") then begin
                            ...
                            if CustLedgerEntry.FindFirst then begin
                                // MyTaxi.W1.CRE.AR01.004 <<
                                if Abs(Difference) <= Abs(CustLedgerEntry."Remaining Amount") then begin
                                    // MyTaxi.W1.CRE.AR01.004 >>
                                ...
                                CustLedgerEntry.SetRange("Document No.", "Bank Acc. Reconciliation Line"."Document No.");
                                CustLedgerEntry.SetRange(Open, true);
                                CustLedgerEntry.SetAutoCalcFields("Remaining Amount");
                                if CustLedgerEntry.FindFirst then begin
                                    // MyTaxi.W1.CRE.AR01.004 <<
                        ...
                        // MyTaxi.W1.CRE.AR01.002 <<
                            ...
                            if CustLedgerEntry.Count > 1 then
                                CustLedgerEntry.SetRange("Remaining Amount", Difference);
                            CustLedgerEntry.SetAutoCalcFields("Remaining Amount");
                            if CustLedgerEntry.FindFirst then begin
                                GenJnlLine.Validate(GenJnlLine."Account Type", GenJnlLine."Account Type"::Customer);
                                ...
                                if Abs(Difference) <= Abs(CustLedgerEntry."Remaining Amount") then begin
                                    ...
                                    CustLedgerEntry.Validate("Amount to Apply", Difference);
