report 70001 "Match Bank Entries MyTaxi"
{
    ...
    begin
        with pBankAccReconciliationLine do begin
            if ("Bank Acc. Ledg. Entry No." = 0) or ("Bal. Account Type" <> "Bal. Account Type"::"G/L Account") then
                exit;
            ...
            if i > 0 then
                for j := 1 to i do begin
                    ...
                    if CustLedgerEntry.FindFirst then begin
                        ...
                        GenJnlLine.Validate(GenJnlLine."Account No.", CustLedgerEntry."Customer No.");
                        if Abs("Statement Amount") <= Abs(CustLedgerEntry."Remaining Amount") then begin
                            if Abs("Statement Amount") = Abs(CustLedgerEntry."Remaining Amount") then
                                "Applyment Confidence" := "Applyment Confidence"::High
                            else
                                "Applyment Confidence" := "Applyment Confidence"::Medium;
                            GenJnlLine.Validate("Applies-to ID", ApplymentID);
                            CustLedgerEntry.Validate("Applies-to ID", ApplymentID);
                            CustLedgerEntry.Validate("Amount to Apply", "Statement Amount");
                            CustLedgerEntry.Modify;
            ...
            if GenJnlLine."Account No." = '' then begin
                ...
                if "Related-Party Name" <> '' then begin
                    Vendor.Reset;
                    Vendor.SetFilter(Name, '%1', '@*' + "Related-Party Name" + '*');
                    if Vendor.FindFirst then
                        bVendorFound := true
                    else begin
                        MyTaxiCRMInterfaceRecords.Reset;
                        MyTaxiCRMInterfaceRecords.SetFilter(name, '%1', '@*' + "Related-Party Name" + '*');
                ...
                if not bVendorFound and ("Related-Party Bank Acc. No." <> '') then begin
                    Vendor.SetRange(Name);
                    VendorBankAccount.Reset;
                    VendorBankAccount.SetFilter(IBAN, '%1', '*' + "Related-Party Bank Acc. No." + '*');
                ...
                if bVendorFound then begin
                    ...
                    if "Document No." <> '' then begin
                        VendorLedgerEntry.SetRange("External Document No.", "Document No.");
                        if not VendorLedgerEntry.FindFirst then
                            VendorLedgerEntry.SetRange("External Document No.", DelChr("Document No.", '<', '0'));
                    end;
                    ...
                    VendorLedgerEntry.SetRange("Remaining Amount", "Statement Amount" - 1, "Statement Amount" + 1);
                    if not VendorLedgerEntry.FindFirst then
                        VendorLedgerEntry.SetRange("External Document No.");
                    if VendorLedgerEntry.FindFirst then
                        if (Abs("Statement Amount") <= Abs(VendorLedgerEntry."Remaining Amount")) then//AND
                                                                                                      //(ABS("Statement Amount")>=ABS(VendorLedgerEntry."Remaining Amount")) THEN
                          begin
                            if Abs("Statement Amount") = Abs(VendorLedgerEntry."Remaining Amount") then
                                "Applyment Confidence" := "Applyment Confidence"::High
                            else
                                "Applyment Confidence" := "Applyment Confidence"::Medium;
                            GenJnlLine.Validate("Applies-to ID", ApplymentID);
                            VendorLedgerEntry.Validate("Applies-to ID", ApplymentID);
                            VendorLedgerEntry.Validate("Amount to Apply", "Statement Amount");
                    ...
                    if "Related-Party Name" <> '' then begin
                        Customer.Reset;
                        Customer.SetFilter(Name, '%1', '@*' + "Related-Party Name" + '*');
                        if Customer.FindFirst then
                            bCustomerFound := true;
                    end;
                    if not bCustomerFound and ("Related-Party Bank Acc. No." <> '') then begin
                        Customer.SetRange(Name);
                        CustomerBankAccount.Reset;
                        CustomerBankAccount.SetFilter(IBAN, '%1', '*' + "Related-Party Bank Acc. No." + '*');
                        ...
                        if CustLedgerEntry.FindFirst then begin
                            if Abs("Statement Amount") <= Abs(CustLedgerEntry."Remaining Amount") then begin
                                if Abs("Statement Amount") = Abs(CustLedgerEntry."Remaining Amount") then
                                    ...
                                    "Applyment Confidence" := "Applyment Confidence"::Low;
                                GenJnlLine.Validate("Applies-to ID", ApplymentID);
                                CustLedgerEntry.Validate("Applies-to ID", ApplymentID);
                                CustLedgerEntry.Validate("Amount to Apply", "Statement Amount");
                                CustLedgerEntry.Modify;
            ...
            if GenJnlLine."Account No." = '' then begin
                ...
                if GenJnlBatch2."No. Series" <> '' then
                    GenJnlLine."Document No." := NoSeriesMgt.GetNextNo(
                        GenJnlBatch2."No. Series", "Transaction Date", false)
                else
            ...
            "New Bal. Account Type" := GenJnlLine."Account Type";
            "New Bal. Account No." := GenJnlLine."Account No.";
            Applied := GenJnlLine.IsApplied;
            Modify;
        end;
