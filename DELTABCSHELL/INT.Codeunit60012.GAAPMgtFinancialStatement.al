codeunit 60012 "GAAP Mgt. -Financial Statement"
{
    ...
    begin
        with ptmpFinancialStatementLine do begin
            Reset;
            DeleteAll;
            ...
            repeat
                ...
                if ("Line Type" = "Line Type"::Posting) and (Code <> '') then begin
                    ...
                    Get(gcodFinStatStructureCode, lintLineNoPosting);
                    "Start Balance" := ldecStartBalanceSubtotal;
                    "End Balance" := ldecEndBalanceSubtotal;
                    "Prior Period Balance" := ldecPriorPeriodSubtotal;
                    Modify;
                end;
        ...
        with ptmpFinancialStatementLine do begin
            ...
            if ltmpGLAcc.FindSet then begin
                repeat
                    ...
                    "G/L Account No." := ltmpGLAcc."No.";
                    Code := precFinancialStatementLineCurr.Code;

                    Description := "G/L Account No." + ' ' + ltmpGLAcc.Name; // MP 10-05-12 Added "G/L Account No."
                    if ltmpGLAcc."Name (English)" <> '' then  // MP 10-05-12 Added condition and "G/L Account No." below
                        "Description (English)" := "G/L Account No." + ' ' + ltmpGLAcc."Name (English)";
                    ...
                    pdecStartBalanceSubtotal += "Start Balance";
                    pdecEndBalanceSubtotal += "End Balance";
                    pdecPriorPeriodSubtotal += "Prior Period Balance";
    ...
    begin
        with ptmpFinancialStatementLine do begin
            ...
            if "Row No." <> '' then // May be included in formula using previous period figure, i.e. [ROWNO]
            ...
            if ltmpCorpGLAcc.FindSet then begin
                repeat
                    ...
                    if (lcodPrevLocalGLAccNo <> ltmpCorpGLAcc."Local G/L Account No.") and (lintLineNoLocalGLAcc <> 0) then begin
                        Get(gcodFinStatStructureCode, lintLineNoLocalGLAcc);
                        "Start Balance" := ldecStartBalanceSubtotalLocal;
                        "End Balance" := ldecEndBalanceSubtotalLocal;
                        "Prior Period Balance" := ldecPriorPeriodSubtotalLocal;
                        Modify;
                    ...
                    "G/L Account No." := ltmpCorpGLAcc."Local G/L Account No.";
                    ...
                    if lcodPrevLocalGLAccNo <> ltmpCorpGLAcc."Local G/L Account No." then begin
                        ...
                        lintLineNoLocalGLAcc := "Line No.";
                    end;

                    "Corporate G/L Account No." := ltmpCorpGLAcc."No.";

                    Description := "Corporate G/L Account No." + ' ' + ltmpCorpGLAcc.Name; // MP 10-05-12 Added "Corporate G/L Account No."
                    if ltmpCorpGLAcc."Name (English)" = '' then // MP 10-05-12 Added condition "Corporate G/L Account No."
                        "Description (English)" := ''
                    else
                        "Description (English)" := "Corporate G/L Account No." + ' ' + ltmpCorpGLAcc."Name (English)";
                    ...
                    ldecStartBalanceSubtotalLocal += "Start Balance";
                    ldecEndBalanceSubtotalLocal += "End Balance";
                    ldecPriorPeriodSubtotalLocal += "Prior Period Balance";
        ...
        with ptmpFinancialStatementLine do begin
            ...
            if precCorpGLAcc.FindSet then
                repeat
                    ...
                    "Line Type" := "Line Type"::Posting;
                    "Corporate G/L Account No." := precCorpGLAcc."No.";
                    "G/L Account No." := precCorpGLAcc."Local G/L Account No.";
                    Code := precFinancialStatementLineCurr.Code;

                    Description := "Corporate G/L Account No." + ' ' + precCorpGLAcc.Name; // MP 10-05-12 Added "Corporate G/L Account No."
                    if precCorpGLAcc."Name (English)" <> '' then // MP 10-05-12 Added condition and "Corporate G/L Account No." below
                        "Description (English)" := "Corporate G/L Account No." + ' ' + precCorpGLAcc."Name (English)";
    ...
    begin
        with ptmpFinancialStatementLine do begin
            SetRange("Line Type", "Line Type"::Totalling);
            if FindSet then begin
                ...
                SetCurrentKey(Code, "Corporate G/L Account No.");
                SetRange("Corporate G/L Account No.", '');
                SetRange("G/L Account No.", '');
                ltmpFinancialStatementLineTot.FindSet;
                repeat
                    SetRange(Type, ltmpFinancialStatementLineTot.Type);
                    SetFilter(Code, ltmpFinancialStatementLineTot."Totalling/Formula");

                    CalcSums("Start Balance", "End Balance", "Prior Period Balance");
                    ldecStartBalanceTotal := "Start Balance";
                    ldecEndBalanceTotal := "End Balance";
                    ldecPriorPeriodTotal := "Prior Period Balance";

                    SetRange(Code);

                    Get(gcodFinStatStructureCode, ltmpFinancialStatementLineTot."Line No.");
    ...
    begin
        with ptmpFinancialStatementLine do begin
            SetRange("Line Type", "Line Type"::Formula);
            if FindSet then begin
                repeat
                ...
                repeat
                    ...
                    Get(ltmpFinancialStatementLineFor."Financial Stat. Structure Code", ltmpFinancialStatementLineFor."Line No.");
        ...
        with ptmpFinancialStatementLine do begin
            ...
            repeat
                case "Line Type" of
                    ...
                    "Line Type"::"End-Total":
                        begin
                            "Start Balance" := ldecStartBalanceTotal[lintLevel];
                            "End Balance" := ldecEndBalanceTotal[lintLevel];
                            "Prior Period Balance" := ldecPriorPeriodTotal[lintLevel];
                            Modify;
                        ...
                        if lintLevel > 0 then
                            for lintI := 1 to lintLevel do begin
                                ldecStartBalanceTotal[lintI] += "Start Balance";
                                ldecEndBalanceTotal[lintI] += "End Balance";
                                ldecPriorPeriodTotal[lintLevel] += "Prior Period Balance"
                            end;
                end;
            until Next = 0;
            ...
            if gblnBottomUp then
                SetRange("Corporate G/L Account No.");
            // MP 21-05-14 <<
            SetRange("G/L Account No."); // MP 09-05-12 Changed from "Corporate G/L Account No."
        end;
    end;
    ...
    begin
        with ptmpFinancialStatementLine do begin
            if precFinancialStatementLineCurr."Show Opposite Sign" then begin
                "Start Balance" := -pdecStartBalance;
                "End Balance" := -pdecEndBalance;
                "Prior Period Balance" := -pdecPriorPeriodBalance;
            end else begin
                "Start Balance" := pdecStartBalance;
                "End Balance" := pdecEndBalance;
                "Prior Period Balance" := pdecPriorPeriodBalance;
            ...
            if (grecFinStatStructure."Rounding Factor" <> grecFinStatStructure."Rounding Factor"::None) and
              not ("Line Type" in ["Line Type"::"End-Total", "Line Type"::Totalling, "Line Type"::Formula]) // MP 05-05-14 Added condition
            then begin
                Evaluate(ldecRoundingPrecision, Format(grecFinStatStructure."Rounding Factor"));

                "Start Balance" := Round("Start Balance" / ldecRoundingPrecision, 1);
                "End Balance" := Round("End Balance" / ldecRoundingPrecision, 1);
                "Prior Period Balance" := Round("Prior Period Balance" / ldecRoundingPrecision, 1);
            end;
        end;
        // MP 03-12-13 <<
    end;
    ...
    begin
        with ptmpFinancialStatementLine do begin
            if "Corporate G/L Account No." <> '' then begin
                ...
                lrecGLEntry.SetRange("Corporate G/L Account No.", "Corporate G/L Account No.");
            end else
                if "G/L Account No." <> '' then begin
                    lrecGLEntry.SetCurrentKey("G/L Account No.", "Posting Date");
                    lrecGLEntry.SetRange("G/L Account No.", "G/L Account No.");
                end else
                    exit;

            if pintFieldNo = FieldNo("Start Balance") then
            ...
            lmdlAdjmtsMgt.gfcnShowEntries(lrecGLEntry, gblnIncludeUnpostedEntries, "Corporate G/L Account No." <> ''); // MP 18-11-15 Replaces above line
        end;
    end;
    ...
    begin
        with precFinancialStatementLine do begin
            ...
            if FindSet then
                repeat
                    lpagNVLookup.AddItem("Row No.", "Description (English)");
                until Next = 0;
    ...
    begin
        ...
        if goptCorpStatementType = goptCorpStatementType::"Tax Financial Statement" then begin
            lrecGenJnlTemplate.SetRange(Type, lrecGenJnlTemplate.Type::Assets);
            lrecGenJnlTemplate.FindFirst;
            ...
            if ltmpGLAcc.FindSet then begin
                repeat
                    ...
                    if (lcodPrevCorpGLAccNo <> ltmpGLAcc."Corporate G/L Account No.") and (lintLineNoCorpGLAcc <> 0) then begin
                        Get(gcodFinStatStructureCode, lintLineNoCorpGLAcc);
                        "Start Balance" := ldecStartBalanceSubtotalCorp;
                        "End Balance" := ldecEndBalanceSubtotalCorp;
                        "Prior Period Balance" := ldecPriorPeriodSubtotalCorp;
                        Modify;
                    ...
                    if lcodPrevCorpGLAccNo <> ltmpGLAcc."Corporate G/L Account No." then begin
                        ...
                        "Corporate G/L Account No." := ltmpGLAcc."Corporate G/L Account No.";

                        Description := "Corporate G/L Account No." + ' ' + ltmpCorpGLAcc.Name;
                        ...
                        lintLineNoCorpGLAcc := "Line No.";

                        "Corporate G/L Account No." := '';
                    end;

                    "G/L Account No." := ltmpGLAcc."No.";

                    Description := "G/L Account No." + ' ' + ltmpGLAcc.Name;
                    ...
                    ldecStartBalanceSubtotalCorp += "Start Balance";
                    ldecEndBalanceSubtotalCorp += "End Balance";
                    ldecPriorPeriodSubtotalCorp += "Prior Period Balance";
