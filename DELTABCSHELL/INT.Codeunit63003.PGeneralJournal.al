codeunit 63003 "P:General Journal"
{
    ...
    begin
        with precGenJournalLine do begin
            if lfcnUseReadyToPost("Journal Template Name") then begin
                if not Confirm(StrSubstNo(txt60000, FieldCaption("Ready to Post")), false) then
                    Error('');

                SetRange("Ready to Post", true);
            end;

            lrecGenJournalBatch.Get("Journal Template Name", "Journal Batch Name");
